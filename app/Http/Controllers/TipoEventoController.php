...
class TipoEventoController extends Controller
{
	...
	public function index(){
		$consultaAtivo = TipoEvento::Where('empresa_id', Auth::user()->empresa_id)
													->where('status', 1)
													->orderBy('id')
													->paginate(10);
		$consultaInativo = TipoEvento::Where('empresa_id', Auth::user()->empresa_id)
													->where('status', 0)
													->orderBy('id')
													->paginate(10);
		return view('Admin.tipoevento.listagem', compact('consultaAtivo', 'consultaInativo'));
	}
